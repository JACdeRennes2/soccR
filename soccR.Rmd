...
ggplot(df_buteurs_top5, aes(x = reorder(Buteur, `Buts[22]`), y = `Buts[22]`)) +
  ...
  labs(title = "Histogramme du nombre de buts des 10 meilleurs buteurs",
       x = "Joueur",
       y = "Nombre de buts")
...
# Liste de correspondance entre clubs et couleurs de maillot
colors <- list("Juventus FC" = "#000000",
               "Manchester City" = "#75AADB",
...
df_passeurs <- read.csv("data/top5_championnats_passeurs.csv", header = TRUE, sep = ",")

# Sélection des données
df_passeurs_top5 <- df_passeurs %>%
  filter(Rang <= 5) %>%
  select(Passeur, Passes = Passes[23])

# Histogramme
ggplot(df_passeurs_top5, aes(x = Passeur, y = Passes, fill = Passeur)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = colors) +
  labs(title = "Histogramme des passes par joueur des 5 meilleurs passeurs",
       x = "Joueur",
       y = "Passes") +
...
```{r message=FALSE}
css_selector <- "#mw-content-text > div.mw-parser-output > table:nth-child(135) > tbody > tr > td:nth-child(1) > table > tbody"
df_buteurs_equipes <- data_html %>% html_nodes(css_selector) %>% html_table()
datatable(df_buteurs_equipes[[1]])
```
...
En aggrégeant, on obtient le nombre de passes décisives par équipe.
...
>>>>>>> 77ceb84dfb30d7ac683fa93e423b5542b0a14d67
