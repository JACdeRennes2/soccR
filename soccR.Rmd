...
ggplot(matches_scores, aes(x = homeTeam_name, y = awayTeam_name, fill = winner)) + 
...
ggplot(matches_bracket, aes(y = team, x = round, fill = winner)) +
  geom_bracket(aes(xmin = round, xmax = lead(round), ymin = team, ymax = team, color = winner),
               height = 0.2, width = 0.4, label.padding = unit(1, "mm")) +
  scale_fill_manual(values = c("red", "blue", "grey"), name = "Winner") +
  scale_color_manual(values = c("red", "blue", "grey"), guide = FALSE) +
  theme_minimal() +
